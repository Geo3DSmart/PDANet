...
from .utils import point_to_voxel, voxel_to_point
from .modules.layers import (DynamicConvolutionBlock,
...
class SPVNAS(RandomNet):
    ...
    def __init__(self, num_classes, macro_depth_constraint, **kwargs):
        ...
        self.classifier = DynamicLinear(232, num_classes)
    ...
    # def determinize(self):
        ...
        sample_feat = torch.randn(1000, 4).cuda()
        sample_coord = torch.randn(1000, 4).random_(997).cuda()
        sample_coord[:, -1] = 0

        if torch.cuda.is_available():
            x = SparseTensor(sample_feat,
                             sample_coord.int())# .to('cuda:%d' % local_rank)  # .cuda()
        else:
            x = SparseTensor(sample_feat, sample_coord.int())
        with torch.no_grad():
            x = self.forward(x.cuda())
    ...
    def forward(self, x):
        ...
        if torch.isnan(x.F).any() or torch.isinf(x.F).any(): 
            print("out")
        elif torch.isnan(x.C.float()).any() or torch.isinf(x.C.float()).any(): 
        ...
        cluster_feature = self.proj(z1.F) # cluster_feature:N,C
        ...
        self.classifier.set_in_channel(z1.F.shape[-1])
        out = self.classifier(z1.F)
        # import pdb
        # pdb.set_trace()
        # cluster_feature = self.proj(z3.F)
        # z1.F:torch.Size([120000, 232])    # 后面部分 torch.Size([1, 64, 120000])
        # out.F:torch.Size([120000, 5])    # 后面部分 torch.Size([1, 64, 120000])
        # return out, cluster_feature.unsqueeze(0).permute(0, 2, 1).contiguous()
        # if torch.isnan(out).any() or torch.isinf(out).any(): 
        #     print("out")
        # elif torch.isnan(cluster_feature).any() or torch.isinf(cluster_feature).any(): 
        #     print("cluster_feature")
        
        return out, cluster_feature  # 前者是类别，后者是特征
