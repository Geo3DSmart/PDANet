...
import os
import json
from .cluster.spvnas_cluster import SPVNAS
...
class IASSD_Backbone(nn.Module):
    ...
    def __init__(self, model_cfg, num_class, input_channels, **kwargs):
        ...
        # channel_out_list = [64]
        # channel_out_list = [232]

        self.num_points_each_layer = []

        sa_config = self.model_cfg.SA_CONFIG
        self.layer_types = sa_config.LAYER_TYPE  # ['SA_Layer', 'SA_Layer', 'SA_Layer', 'SA_Layer', 'Vote_Layer', 'SA_Layer']
        self.ctr_idx_list = sa_config.CTR_INDEX  # [-1, -1, -1, -1, -1, 5]
        self.layer_inputs = sa_config.LAYER_INPUT  # [0, 1, 2, 3, 4, 3]
        self.aggregation_mlps = sa_config.get('AGGREGATION_MLPS', None) # [[64], [128], [256], [256], [], [512]]
        self.confidence_mlps = sa_config.get('CONFIDENCE_MLPS', None)   # [[], [128], [256], [], [], []]
        self.max_translate_range = sa_config.get('MAX_TRANSLATE_RANGE', None)  # [3.0, 3.0, 2.0]
        ...
        for k in range(sa_config.NSAMPLE_LIST.__len__()):
            ...
            if self.layer_types[k] == 'SA_Layer':
                ...
                if k<1 or k >4 :
                    ...
                    self.SA_modules.append(
                        pointnet2_modules.PointnetSAModuleMSG_WithSampling_Ellipsoid(
                        # pointnet2_modules.PointnetSAModuleMSG_WithSampling_Ellipsoid_No_Global(
                            npoint_list=sa_config.NPOINT_LIST[k],
                            sample_range_list=sa_config.SAMPLE_RANGE_LIST[k],
                            sample_type_list=sa_config.SAMPLE_METHOD_LIST[k],
                            radii=sa_config.RADIUS_LIST[k],
    ...
    def forward(self, batch_dict):
        ...
        # points = torch.cat([points,cluster_feature],dim=-1)

        batch_idx, xyz, features = self.break_up_pc(points)  # start：points: [B*N,5]
        ...
        for i in range(len(self.SA_modules)):
            ...
            if self.layer_types[i] == 'SA_Layer':
                ...
                li_xyz, li_features, li_cls_pred,sample_list_id = self.SA_modules[i](xyz_input, feature_input, li_cls_pred, ctr_xyz=ctr_xyz)
            ...
            # encoder_sample_list_id.append(sample_list_id)
            if len(sample_list_id)!=0:
                encoder_sample_list_id.append(sample_list_id) # B,score(1),N  B,N
            else:
                encoder_sample_list_id.append(sample_list_id)

            if li_cls_pred is not None:
                li_cls_batch_idx = batch_idx.view(batch_size, -1)[:, :li_cls_pred.shape[1]]  # B,N,1+Class
                sa_ins_preds.append(torch.cat([li_cls_batch_idx[..., None].float(),li_cls_pred.view(batch_size, -1, li_cls_pred.shape[-1])],dim =-1)) 
                # sample_list_id= sample_list_id.unsqueeze(1)
            else:
                sa_ins_preds.append([])
                # encoder_sample_list_id.append([])
        ...
        # batch_dict['centers'] = batch_dict['ctr_offsets'] +batch_dict['centers_origin']
        ...
        batch_dict['sample_list_id'] = encoder_sample_list_id
        ...
        if self.model_cfg.SA_CONFIG.get('SAVE_SAMPLE_LIST',False) and not self.training:  
            import numpy as np 
            result_dir = np.load('/mnt/data/**/code/3ddetection/S_semant_model/IA-SSDAddModel/tools/tmp.npy', allow_pickle=True)
            for i in range(batch_size)  :
                ...
                xyz_list = []
                for sa_xyz in encoder_xyz: # 对每个输出的xyz进行保存
                    xyz_list.append(sa_xyz[i].cpu().numpy()) 
                if '/' in idx: # Kitti_tracking  没有用
