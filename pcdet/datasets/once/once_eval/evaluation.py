...
from .eval_utils import compute_split_parts, overall_filter, distance_filter, overall_distance_filter,overall_distance_filter_IASSD
...
def get_evaluation_results(gt_annos, pred_annos, classes,
    ...
    if difficulty_mode == 'Distance':
        ...
        num_difficulties = 4  #原始的
        difficulty_types = ['overall', '0-30m', '30-50m', '50m-inf']
        
        # num_difficulties = 5
        # difficulty_types = ['overall', '0-30m', '30-50m', '50m-70m', '70m-inf']
    else:
    ...
    for cls_idx, cur_class in enumerate(classes):
        ...
        for diff_idx in range(num_difficulties): # 不同的距离程度
            ...
            for sample_idx in range(num_samples):
                ...
                # # IASSD
                # gt_flag, pred_flag = filter_data_IASSD(gt_anno, pred_anno, difficulty_mode,
                #                                     difficulty_level=diff_idx, class_name=cur_class, use_superclass=use_superclass)
...
def filter_data(gt_anno, pred_anno, difficulty_mode, difficulty_level, class_name, use_superclass):
    ...
    if difficulty_mode == 'Overall':
        ...
        ignore = overall_distance_filter(gt_anno['boxes_3d'], difficulty_level) # 不同距离的结果
        gt_flag[ignore] = 1
...
def filter_data_IASSD(gt_anno, pred_anno, difficulty_mode, difficulty_level, class_name, use_superclass):
    """
    Filter data by class name and difficulty
    ...
    if difficulty_mode == 'Overall':
        ...
        ignore = overall_distance_filter_IASSD(gt_anno['boxes_3d'], difficulty_level) # 不同距离的结果
        gt_flag[ignore] = 1
        ignore = overall_distance_filter_IASSD(pred_anno['boxes_3d'], difficulty_level)
        pred_flag[ignore] = 1
    else:
        raise NotImplementedError
