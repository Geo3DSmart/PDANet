...
__global__ void query_ellipsoid_point_kernel(int b, int n, int m, float e1, float e2, float e3,
    ...
    v += m*3*3*batch_index;
    d += m*3*batch_index;
    // distance += m * nsample * batch_index;
    ...
    for (int j = index; j < m; j += stride) {
      ...
      if((ingroup_pts_cnt[j]>=3)){
        ...
        float deter = (*(v+(c*c*j)+6))*((*(v+(c*c*j)+4))*(*(v+(c*c*j)+2)) -(*(v+(c*c*j)+1))*(*(v+(c*c*j)+5))) - (*(v+(c*c*j)+7))*((*(v+(c*c*j)+3))*(*(v+(c*c*j)+2))-(*(v+(c*c*j)+0))*(*(v+(c*c*j)+5))) + (*(v+(c*c*j)+8)) * ((*(v+(c*c*j)+3))*(*(v+(c*c*j)+1))-(*(v+(c*c*j)+0))*(*(v+(c*c*j)+4)));
        for (int k=0;k<n;++k) {
          if (cnt == nsample)
          ...
          float d3=sqrtf((xx*xx/aa)+(yy*yy/bb)+(zz*zz/cc));
...
void query_ellipsoid_point_kernel_wrapper(int b, int n, int m, float e1, float e2, float e3,
    ...
    query_ellipsoid_point_kernel<<<b, opt_n_threads(m), 0, stream>>>(
      b, n, m, e1, e2, e3, nsample, new_xyz, xyz, idx, ingroup_pts_cnt, ingroup_out, ingroup_cva, v, d);

    CUDA_CHECK_ERRORS();//opt_n_threads(m)
}
