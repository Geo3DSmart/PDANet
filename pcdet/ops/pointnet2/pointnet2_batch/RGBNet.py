...
class _PointnetSAModuleBase(nn.Module):
    ...
    # samples_mask： B,1,N  points_xyz:(B, N, 3) 
    ...
    def forward(self, xyz: torch.Tensor, features: torch.Tensor = None, new_xyz=None):
        ...
        if new_xyz is None:
            if not self.fbs_use:
                ...
                sample_masks = torch.zeros_like(confidence_score_margin).long()  # B,  1, num_points
                # 对每个B中的点，挑选出来的值赋1
                for b in range(sample_masks.shape[0]):
                    sample_masks[b][top_indices[b]] = 1
                # 进行特征点采样点 self.fg_nsample 为采样的点数
        ...
        for i in range(len(self.groupers)): # 聚集特征
            if self.zero_query:
